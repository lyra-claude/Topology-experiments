...
rugged landscapes.
While our categorical framework (the confound theorem) motivates the
investigation of directed cycle count as a topological invariant, the core
contributions of this paper are empirical: controlled experiments demonstrating
that topology affects evolutionary dynamics at constant density, that this
effect scales with landscape ruggedness, and that diversity maintenance is the
primary mediating channel. We view the theoretical explanation of \emph{why}
cycles modulate diversity as an open problem for future work.
...
Recent work on cellular sheaves~\cite{hanks2025cellular} shows that $\beta_1$
can be understood as the dimension of sheaf cohomology $H^1(G;\mathcal{F})$,
providing a natural extension to heterogeneous agent capabilities.
...
Dochkina et al.~\cite{dochkina2026}
  & 44\% quality spread ($d{=}1.86$); capability moderates
...
cycles are harmful. On the cost side, cycles do impose overhead: iterative
message-passing until convergence, rather than the single-pass execution of
DAGs. Our experiments quantify the \emph{benefit} side---sustained diversity on
rugged landscapes---completing a cost-benefit framework for topology choice.
...
\paragraph{Joint prompt--topology optimization.}
MASS~\cite{zhou2025mass} is the closest predecessor: it jointly optimizes
prompts and topologies for multi-agent systems via a three-stage pipeline
(block-level prompts, workflow topology, workflow-level prompts). However,
MASS restricts its topology search to DAGs, and its central finding---that
``prompts matter more than topology''---may be an artifact of this
restriction. By Theorem~\ref{thm:confound}, varying DAG structure at
constant node count simultaneously varies density. With cycles excluded from
the search space, the dominant topological variable ($\kappa$) is held at
zero, artificially suppressing topology's explanatory power. Our experiments
show that when $\kappa$ is allowed to vary at constant density, topology
explains up to 69\% of diversity variance on rugged landscapes.
...
conduct the largest controlled topology study to date: 25,000 tasks,
256 agents, and 8 coordination protocols yield a 44\% quality spread
(Cohen's $d = 1.86$), demonstrating that topology choice is the
dominant performance lever at industrial scale. Crucially, the effect
is capability-moderated: weaker models benefit most from DAG
structure, while stronger models exploit cyclic communication
patterns---suggesting a Goldilocks interaction between agent
capability and cycle count. However, sparsity and
acyclicity are confounded in all runs; by Theorem~\ref{thm:confound},
this is a density comparison. Our experiment provides the controlled
condition their design lacks.
Wang et al.~\cite{wang2026kregular} find that $K$-regular graphs
maximise coordination quality, with a minimum capability threshold
$\beta_{\min} \sim 1/K$: higher connectivity substitutes for
individual agent intelligence. This highlights a
coordination--exploration duality---density aids coordination but, as
our experiments show, suppresses diversity-mediated exploration. The
intersection defines an optimal topology that balances both.

\paragraph{Task structure and topology generation.}
Amir et al.~\cite{amir2025schur} prove that Schur-convexity of a task's
performance function determines \emph{whether} agent diversity improves
outcomes, independent of the communication topology. Their result is the
task-side dual of our framework: $\kappa$ (and more broadly $\beta_1$)
governs how much diversity the topology creates, while their Schur-convexity
parameter~$\sigma$ governs whether that diversity is beneficial at all.
Together, the two axes yield a practical design rule: measure~$\sigma$ first
to decide if diversity helps, then tune~$\kappa$ to supply the right amount.
Complementarily, TopoDIM~\cite{topodim2025} learns a task-to-topology mapping
that generates communication graphs from task requirements, achieving 46\%
token savings over static topologies. This is the inverse of our analysis:
we characterise how topology affects performance; they generate topology from
performance objectives. Our theoretical framework---particularly the
density--cycle confound and the role of $\kappa$---could inform the
topological invariants their learned functor targets.
